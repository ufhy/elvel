...
import { config, controller } from '@elvel/core'
 ...
 * `spa.embed` off the document carries nothing — the same bytes for everybody, so a
 * cache may keep it — which means the two things every page needs have to be asked
 * for instead: who this is, and the CSRF token for this session.
 *
 ...
 * shell carries no CSRF token — a token is per session, and a document carrying one
 * could not be cached — so without an unguarded way to fetch it the sign-in form has
 * nothing to post. Measured as `419 CSRF token mismatch` on a fresh visit.
 *
 * `user: null` is a real answer here, not a failure. What guards the application is
 * the document route: `spa.areas` refuses a guest `/dashboard` before any of its
 * JavaScript loads, so nothing behind this endpoint depends on it saying no.
 */
export default controller('api-session').get('/api/session', () => {
  const person = user()
