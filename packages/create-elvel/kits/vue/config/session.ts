...
export default {
  ...
  /**
   * Paths exempt from CSRF, and this kit narrows the template's `/api/*`.
   *
   * Every write this application answers lives under `/api` — `routes/auth.ts` and
   * `routes/settings.ts` put them there so one address never means both a screen
   * and a form post. With the template's exemption that would have taken CSRF off
   * signing in, changing a password, revoking a session and closing an account, all
   * at once. Measured: `POST /api/sign-in` with no token answered `302` instead of
   * `419`.
   *
   * So only better-auth's own mount stays out. Its browser client posts to
   * `/api/auth/*` and knows nothing about this application's token — the passkey
   * flows go through it — while everything of ours is sent by `lib/form.ts`, which
   * attaches the token from `GET /api/session` on every write.
   */
  csrfExcept: ['/api/auth/*']
}
