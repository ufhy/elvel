 ...
 * Once published to npm:           bun create elyvel my-app
 *
...
async function main(): Promise<number> {
  ...
  prompts.intro(pc.bgCyan(pc.black(' create-elyvel ')))
 ...
 * installed. `create-elyvel.test.ts` holds this list to the contents of
...
async function findMonorepoRoot(): Promise<string | undefined> {
  ...
  while (true) {
    ...
    if (await manifest.exists()) {
      ...
      if (parsed.name === 'elyvel' && parsed.workspaces !== undefined) return directory
    }
 ...
 * through `@elyvel/http`, not through an import of its own. `@kitajs/html` is
 ...
 *
 * A copy of `OWNERS` in `@elyvel/console`'s `config:publish`, held to it by a
 * test rather than imported: this installer depends on no framework package, so
 * that `bunx create-elyvel` downloads a scaffolder and not a framework.
 *
...
async function pruneConfig(target: string, present: Set<string>): Promise<void> {
  ...
  for (const [name, owner] of Object.entries(CONFIG_OWNERS)) {
    if (present.has(`@elyvel/${owner}`)) continue
