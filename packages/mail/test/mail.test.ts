...
describe('building a message', () => {
  test('the envelope and content are resolved into one object', async () => {
    ...
    expect(message.from).toEqual({ address: 'hello@example.com', name: 'Elysian' })
  ...
  test('alwaysTo redirects everything and keeps the originals in headers', async () => {
    ...
    expect(message.headers['X-Elysian-To']).toBe('customer@example.com')
    expect(message.headers['X-Elysian-Bcc']).toBe('audit@example.com')
  })
...
describe('HTTP transports', () => {
  ...
  test('Resend gets the message as JSON', async () => {
    ...
    try {
      ...
      expect(body.from).toBe('"Elysian" <hello@example.com>')
...
describe('MailManager', () => {
  ...
  beforeEach(() => {
    ...
    app.config.set('mail.from', { address: 'hello@example.com', name: 'Elysian' })
