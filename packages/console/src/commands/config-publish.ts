...
export class ConfigPublishCommand extends Command {
  ...
  private async publish(names: string[]): Promise<number> {
    ...
    for (const name of names) {
      ...
      if (!source) {
        ...
        // eleven errors.
        if (names.length === 1) {
          this.error(`[@elvel/${OWNERS[name]}] is not installed, so there is no ${name} config.`)
          this.comment(`Install it first: bun add @elvel/${OWNERS[name]}`)
          failed = true
        }
  ...
  private defaultFor(name: string): string | undefined {
    try {
      return Bun.resolveSync(`@elvel/${OWNERS[name]}/config/${name}.ts`, this.app.basePath())
    } catch {
      return undefined
    }
  }
