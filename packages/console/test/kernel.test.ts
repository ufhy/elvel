...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'elysian-kernel-'))
...
describe('dispatch', () => {
  ...
  test('binds arguments and options', async () => {
    const { status, output } = await run(['app:greet', 'elysian', '--repeat', '2'])

    expect(status).toBe(0)
    expect(output.split('hello elysian').length - 1).toBe(2)
  })
