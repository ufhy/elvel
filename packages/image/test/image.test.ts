...
describe.if(detected !== undefined)(`transformations through ${detected?.name}`, () => {
  ...
  test('store writes the transformed bytes', async () => {
    const path = join(process.env.TMPDIR ?? '/tmp', `elvel-image-test-${process.pid}-stored.png`)
  ...
  test('leaves nothing behind in the temporary directory', async () => {
    const before = new Bun.Glob('elvel-image-*').scanSync({
    ...
    const after = new Bun.Glob('elvel-image-*').scanSync({
      cwd: process.env.TMPDIR ?? '/tmp',
      onlyFiles: false
    })
