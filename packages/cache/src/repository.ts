...
export class Repository {
  ...
  async flexible<T>(
    ...
    const createdKey = `elvel:cache:flexible:created:${key}`
    ...
    const refresh = async () => {
      ...
      await this.store
        .lock(`elvel:cache:flexible:lock:${key}`, lock.seconds ?? 0, lock.owner)
        .get(async () => {
    ...
    defer(refresh, { key: `elvel:cache:flexible:${key}` })
