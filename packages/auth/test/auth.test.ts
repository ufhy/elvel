...
import { BunSqlConnection, QueryBuilder, SchemaBuilder } from '@elysian/database'
import { betterAuth } from 'better-auth'
import { Elysia } from 'elysia'
import { elysianAdapter, migrationFor } from '../src/adapter.ts'
...
function makeAuth(db: never) {
  return betterAuth({
    ...
    database: elysianAdapter(db, { dialect: 'sqlite' })
  })
}

describe('elysianAdapter', () => {
...
describe('migrationFor', () => {
  ...
  test('the generated file is a migration our migrator understands', () => {
    expect(code).toContain("import { Migration, type MigrationContext } from '@elysian/database'")
