...
| Vector/similarity clauses (pgvector) | `whereFullText` is implemented — `MATCH ... AGAINST` on MySQL, `to_tsvector @@ plainto_tsquery` on Postgres, and a refusal with an explanation on SQLite rather than a LIKE that pretends. Vector search still needs its own grammar and its own extension. |
| JSON `->` in **updates** and `whereJsonLength` | Paths in wheres (`where('meta->theme', …)`) and `whereJsonContains`/`whereJsonDoesntContain` are implemented per dialect and tested against all three engines. Updating one key inside a document, and asking an array's length, are the remaining verbs. |
