...
starter kits, configuration, routing, the database, validation, the cache,
queues, encryption, and how to work on the framework itself.

It is built from `apps/docs`, so it ships in the same commit as the code it
describes and cannot drift into describing a version that never existed. Twelve
