...
cache, queues, scheduling, the console, mail, file storage, encryption, testing,
and how to work on the framework itself.

It is built from `apps/docs`, so it ships in the same commit as the code it
describes and cannot drift into describing a version that never existed. Eighteen
